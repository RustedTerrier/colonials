...
fn render(display: bool, colony: &Vec<Colony>,){
    ...
    if display {
        ...
        print!(" {one}/##\\ {two}/##\\\n\r{reset}             ###\n\r",one = color::Fg(color::Red),two = color::Fg(color::Blue),reset = color::Fg(color::Reset));
        ...
        while i < 4 {
            ...
            if colony_s.selected == 0 {
                if colony_s.owner == 0{
                    ...
                    if colony_s.owner == 1{
                        println!("{red}<#########>{reset}  #{red}1{reset}#\n\r             ###\r",red = color::Fg(color::Red),reset = color::Fg(color::Reset));
                    }else{
                        println!("{blue}<#########>{reset}  #{blue}2{reset}#\n\r             ###\r",blue = color::Fg(color::Blue),reset = color::Fg(color::Reset));
                    };
                };
            }else{
                if colony_s.selected == 1 {
                    ...
                    if colony_s.owner == 0{
                        ...
                        if colony_s.owner == 1{
                            ...
                            println!("{red}<#########>{reset}  #{blue}2{reset}#\n\r             ###\r",red = color::Fg(color::Red),blue = color::Fg(color::Blue),reset = color::Fg(color::Reset));
                        };
                    };
                };
            };
